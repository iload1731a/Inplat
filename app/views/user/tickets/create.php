...
<div class="row justify-content-center">
    <div class="col-lg-7">
        <div class="glass rounded-4 p-4">
            ...
            <form id="createTicketForm" data-ajax="true" action="/user/tickets" method="POST" enctype="multipart/form-data">
                        ...
                        <select name="category" class="form-select bg-transparent text-light border-secondary">
                            <?php if ($categories !== []): ?>
                                <?php foreach ($categories as $cat): ?>
                                    <option value="<?= e((string)($cat['slug'] ?? '')) ?>">
                                        <?= e((string)($cat['name'] ?? '')) ?>
                                    </option>
                                <?php endforeach; ?>
                            <?php else: ?>
                ...
                <div class="mb-3">
                    <label class="form-label text-secondary small">Attachment <span class="text-muted">(optional, max 10 MB)</span></label>
