...
final class PlatformRepository
{
    ...
    private function adminModuleStatus(PDO $pdo): array
    {
        ...
        $counts = [
            ...
            'trading_management' => (int)$pdo->query('SELECT (SELECT COUNT(*) FROM trades) + (SELECT COUNT(*) FROM positions)')->fetchColumn(),
            'markets' => (int)$pdo->query('SELECT COUNT(DISTINCT market_type) FROM trading_pairs')->fetchColumn(),
            ...
            'news' => (int)$pdo->query('SELECT COUNT(*) FROM banners')->fetchColumn(),
        ...
        return [
            ...
            ['module' => 'Trading Management', 'total' => $counts['trading_management']],
